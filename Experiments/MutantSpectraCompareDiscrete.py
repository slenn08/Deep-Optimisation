...
for c, e, problem_size in problems:
    ...
    negative_change_DO = 0
    no_change_DO = 0
    positive_change_DO = 0

    negative_change_VDO5 = 0
    no_change_VDO5 = 0
    positive_change_VDO5 = 0

    negative_change_VDO10 = 0
    no_change_VDO10 = 0
    positive_change_VDO10 = 0

    negative_change_VDO15 = 0
    no_change_VDO15 = 0
    positive_change_VDO15 = 0
        ...
        with torch.no_grad():
            ...
            for i in range(len(hidden_repr)):
                ...
                for new_var in [1,-1]:
                    ...
                    negative_change_DO += int(d_fitness < 0)
                    no_change_DO += int(d_fitness == 0)
                    positive_change_DO += int(d_fitness > 0)
        ...
        with torch.no_grad():
            ...
            for i, (mean,stdi) in enumerate(zip(hidden_repr, std)):
                ...
                for new_var in [mean - 5*stdi, mean + 5*stdi]:
                    ...
                    d_fitness = new_fitness - fitness
                    negative_change_VDO5 += int(d_fitness < 0)
                    no_change_VDO5 += int(d_fitness == 0)
                    positive_change_VDO5 += int(d_fitness > 0)
                new_hidden_repr = hidden_repr.clone()
                for new_var in [mean - 10*stdi, mean + 10*stdi]:
                    ...
                    d_fitness = new_fitness - fitness
                    negative_change_VDO10 += int(d_fitness < 0)
                    no_change_VDO10 += int(d_fitness == 0)
                    positive_change_VDO10 += int(d_fitness > 0)
                new_hidden_repr = hidden_repr.clone()
                for new_var in [mean - 15*stdi, mean + 15*stdi]:
                    ...
                    d_fitness = new_fitness - fitness
                    negative_change_VDO15 += int(d_fitness < 0)
                    no_change_VDO15 += int(d_fitness == 0)
                    positive_change_VDO15 += int(d_fitness > 0)
    ...
    DO_changes = [round(positive_change_DO*100,2), round(no_change_DO*100,2),
                  round(negative_change_DO*100,2)]
